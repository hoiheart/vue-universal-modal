(function (global, factory) {
	...
	*/
	...
	var useA11Y = function useA11Y(_ref) {
	  ...
	  function setLastActiveElement(event) {
	    ...
	    if (!isModalEvent || isModalEvent !== modalRef.value) {
	      ...
	      if (isModalEvent && !isModalEvent.classList.contains("".concat(CLASS_NAME, "-show"))) {
	        return;
	      }
	...
	var useClose = function useClose(_ref2) {
	...
	var useOrder = function useOrder(_ref3) {
	  ...
	  var latest = vue.computed(function () {
	    var arr = _toConsumableArray(visibleModals.value.values());
	...
	var script = vue.defineComponent({
	  ...
	  setup: function setup(props, context) {
	    ...
	    var inserted = vue.ref(modelValue.value === undefined ? true : modelValue.value);
	    var modalRef = vue.ref(null);
	    var show = vue.ref(!disabled.value);
	...
	function render(_ctx, _cache, $props, $setup, $data, $options) {
	  return _ctx.inserted ? (vue.openBlock(), vue.createBlock(vue.Teleport, {
	    key: 0,
	    to: _ctx.teleportTarget,
	    disabled: _ctx.disabled
	  }, [vue.createVNode(vue.Transition, vue.mergeProps({
	    appear: "",
	    name: _ctx.CLASS_NAME
	  }, vue.toHandlers(_ctx.onTransitionEmit)), {
	...
	var install = function install(app) {
	  ...
	  var visibleModals = vue.ref(new Set());

	  var addVisibleModals = function addVisibleModals(el) {
	    visibleModals.value.add(el);
	  };

	  var removeVisibleModals = function removeVisibleModals(el) {
	    visibleModals.value.delete(el);
	  };

	  app.provide(PLUGIN_NAME, {
	    teleportTarget: teleportTarget,
	    visibleModals: vue.readonly(visibleModals),
